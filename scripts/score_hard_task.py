"""
score_hard_task.py
Scores all 6 models on the HARDER extraction task (paraphrased descriptions,
no quoted ground-truth spans). Compares against the original task's results
to test whether accuracy drops when the task requires genuine extraction
rather than pattern-matching quoted text.
"""
...
RESULTS_FILE = Path(__file__).parent.parent / "results" / "hard_task_raw_responses.json"
SUMMARY_FILE = Path(__file__).parent.parent / "results" / "hard_task_summary.json"
...
def mcnemar_pairwise(binary_matrix, i, j):
    col_i, col_j = binary_matrix[:, i], binary_matrix[:, j]
...
def main():
    ...
    print(f"Comparisons: {len(pairs)}, Bonferroni alpha: {bonferroni_alpha:.5f}")

    pairwise_results = []
    for i, j in pairs:
        m_i, m_j = model_keys[i], model_keys[j]
        b, c, p = mcnemar_pairwise(binary_matrix, i, j)
        significant = p < bonferroni_alpha
        pairwise_results.append({
            "model_a": MODEL_NAMES[m_i], "model_b": MODEL_NAMES[m_j],
            "a_wrong_b_right": int(b), "a_right_b_wrong": int(c),
            "p_value": round(p, 6), "significant_after_bonferroni": bool(significant),
        })
        sig = "***" if significant else ""
        print(f"{MODEL_NAMES[m_i]} vs {MODEL_NAMES[m_j]}: b={b}, c={c}, p={p:.6f} {sig}")
